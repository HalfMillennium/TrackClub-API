...
track_info = [] // Provided in request
_track_master = []  // Created after processing request
//-------------------------------------------------------------//

// Fetches "currently" playing track from radio
router.get("/fetch-radio", function(request, response) {
...
// Queue
router.get("/load-radio", function (request, response) {
    ...
    curr_dur = track_info[0].duration
    ...
    while(j < track_info.length && (curr_dur > 0 || i == 0)) {
        ...
        } else if(track_master[track_master.length-1]["curr_sum"] % CHUNK_SIZE + curr_dur <= CHUNK_SIZE){
            last = track_master[track_master.length-1]
